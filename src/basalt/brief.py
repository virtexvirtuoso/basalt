...
class Brief:
    ...
    def _render_buried_insight(self, findings: list[BuriedInsight]) -> list[str]:
        ...
        for i, finding in enumerate(findings, 1):
            ...
            lines.append(f"**Age:** {(self.generated_at - cand.created).days} days old, dormant for {(self.generated_at - cand.updated).days} days")
            lines.append("")
            ...
            if finding.validators:
                ...
                for v in finding.validators[:3]:
                    ...
                    lines.append(f"- {link_status} [[{v.title}]]({v.rel_path}) — updated {(self.generated_at - v.updated).days} days ago")
                lines.append("")
        return lines
    ...
    def _render_implicit_thesis(self, findings: list[ThesisCluster]) -> list[str]:
        ...
        for i, cluster in enumerate(findings, 1):
            ...
            lines.append(f"**Cluster Size:** {cluster.cluster_size} notes across {cluster.folder_diversity} folders")
            ...
            for j, (path, title, quote, prov) in enumerate(zip(
                cluster.member_paths, cluster.member_titles,
                cluster.member_quotes, cluster.member_quote_provenances
            ), 1):
                if j == 1:
    ...
    def _render_connection(self, findings: list[ConnectionPair]) -> list[str]:
        ...
        for i, pair in enumerate(findings, 1):
            ...
            lines.append("")
            lines.append(f"**Similarity:** {pair.similarity:.2f} | **Score:** {pair.score:.3f}")
            lines.append("")
    ...
    def _render_drift(self, findings: list[DriftFinding]) -> list[str]:
        ...
        for i, finding in enumerate(findings, 1):
            ...
            lines.append("")
            lines.append(f"**Window:** {finding.window_days} days | **Daily Notes:** {finding.daily_note_count} | **Projects:** {finding.project_count}")
            lines.append("")
            if finding.headline_overworked:
                ow = finding.headline_overworked
                lines.append(f"**Overworked:** {ow.name}")
                lines.append(f"- Stated: #{ow.stated_rank} ({ow.stated_notes} notes, {ow.stated_share*100:.1f}%)")
                lines.append(f"- Lived: #{ow.lived_rank} ({ow.lived_mentions} mentions, {ow.lived_share*100:.1f}%)")
            ...
            if finding.headline_underworked:
                ...
                lines.append(f"- Stated: #{uw.stated_rank} ({uw.stated_notes} notes, {uw.stated_share*100:.1f}%)")
                lines.append(f"- Lived: #{uw.lived_rank} ({uw.lived_mentions} mentions, {uw.lived_share*100:.1f}%)")
    ...
    def _render_contradiction(self, findings: list[ContradictionPair]) -> list[str]:
        ...
        for i, pair in enumerate(findings, 1):
            ...
            lines.append("")
            lines.append(f"**Topical Similarity:** {pair.similarity:.2f} | **Contradiction Score:** {pair.contradiction_score:.1f} | **Signals:** {', '.join(pair.signals)}")
            lines.append("")
            lines.append(f"#### A: {pair.note_a_title}")
            lines.append(f"`{pair.note_a_path}`")
            lines.append("")
            ...
            lines.append(f"> — *{pair.note_b_quote_provenance}*")
            lines.append("")
            lines.append(f"**Verdict:** ⚠️ Review required — these claims may be incompatible")
            lines.append("")
        return lines
